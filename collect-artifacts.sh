...
if [[ "$1" == "-h" || "$1" == "--help" ]]; then
    ...
    echo "Runs artifact collection inside container '$DEFAULT_CONTAINER_NAME'."
...
echo "▶ Collecting artifacts inside container '$DEFAULT_CONTAINER_NAME'..."
docker exec -it "$DEFAULT_CONTAINER_NAME" \
    bash -c "~/alt-syz-box/scripts/tools/collect-artifacts.sh ${EXTRA_ARGS[*]}"
