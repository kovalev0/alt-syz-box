...
# ------------------------------------------------
# drivers_hwmon_jc42
# 8. Ensure I2C stub module is loaded and configure jc42 device on boot
...
echo "jc42 0x18" > /sys/bus/i2c/devices/i2c-${I2C_NUM}/new_device
'
...
if [ ! -f "$RC_LOCAL_SCRIPT" ]; then
    ...
    if ! sudo grep -q '# drivers_hwmon_jc42' "$RC_LOCAL_SCRIPT"; then
        echo "${SCRIPT_BLOCK}" | sudo tee -a "$RC_LOCAL_SCRIPT"
    fi
fi
